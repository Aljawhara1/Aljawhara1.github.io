...
const ll = document.createDocumentFragment();
ll.textContent = 'sadsa';
//make component function
export function component({ type, attr, events, style, $for, push, contentNode, textNode, langs, children }) {
        ...
        for (let i = 0; i < children.loop; i++) {
          ...
          if (children.attr) {
            for (const key of Object.keys(children.attr)) {
              let attrs = children.attr[key] instanceof Function ? children.attr[key]() : children.attr[key]
              if (!attrs[i]) { continue; }
              item.setAttribute(key, attrs[i])
            }
          ...
          //start set contents
          if (children.content) {
            children.content[i] ? item.append(children.content[i]) : item.append('');
  ...
  if (contentNode) {
    if (!type) {
      ...
      contentOfFragment.insertAdjacentHTML('beforeend', contentNode);
      el.appendChild(contentOfFragment)
    }
    else { el.insertAdjacentHTML('beforeend', contentNode) };
  }
  ...
  if (children) {
    for (const key of Object.keys(children)) {
      let childType = children[key].type, childAttr = children[key].attr,
        childContent = children[key].contentNode,
        childTextNode = children[key].textNode,
      ...
      if (childContent) {
        if (Array.isArray(childContent)) {
          for (let i = 0; i < childContent.length; i++) {
            childContent instanceof Element || childContent instanceof DocumentFragment? childEl.appendChild(childContent[i]) : childEl.insertAdjacentHTML('beforeEnd', childContent[i]);
          }
        } else { childContent instanceof Element || childContent instanceof DocumentFragment? childEl.appendChild(childContent) : childEl.insertAdjacentHTML('beforeEnd', childContent) }
      }
